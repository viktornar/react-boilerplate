...
import { mediaQueryMapper } from '../../helpers/mediaQuery';
...
class Columns extends Component {
  ...
  getColumnsContainers({ masonry, children, columns }) {
    const validChildren = children.filter((c) => !isUndefined(c));
  ...
  getColumnClassName = ({ fill, masonry }) => (!masonry && fill ? 'columns-layout__column columns-layout__column--fill' : 'columns-layout__column');
  ...
  render() {
    const { className, children, masonry, fill } = this.props;
    ...
    return (
      <div className={ `${className} columns-layout` }>
        <div className="columns-layout__row">
